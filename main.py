...
from src.model.model import DeepHedging_BS, DeepHedging_Hest, DeepHedging_Hest_GRU
...
def main(logger, conf):
    ...
    if conf["model_init"]["bs_model"]:
        model = DeepHedging_BS(conf).to(device)
        logger.debug("Creating BS RNN model architecture.")
    else:
        model = DeepHedging_Hest_GRU(conf).to(device)
